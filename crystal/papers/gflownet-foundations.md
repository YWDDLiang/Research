...
slug: "gflownet-foundations"
year: 2023
venue: "Journal of Machine Learning Research"
category: "cross-domain"
audit_role: "多模态奖励比例采样"
paper_url: "https://jmlr.org/papers/v24/22-0364.html"
project_url: ""
...
| 字段 | 内容 |
|---|---|
| 年份 / 出处 | 2023 · Journal of Machine Learning Research |
| 主题 | cross-domain |
| 在领域中的角色 | 多模态奖励比例采样 |
| 论文 | [论文地址](https://jmlr.org/papers/v24/22-0364.html) |
| 项目 | 未核验到官方项目地址（不等于不存在） |
| 审计状态 | 已审计；基础方法论文 |

## 0. 三十秒结论

**论文做了什么：** 建立生成流网络的理论基础，使组合对象按未归一化奖励比例采样，而不是只寻找单一最优对象。

**去故事化判断：** 基础思想非常契合多晶型发现，但直接套用并不构成贡献；无机晶体的混合离散—连续、等价性和多保真问题才是难点。

**对当前研究最直接的用途：** 作为 energy-only GRPO 的替代目标候选，用于保持低能 basin 的概率覆盖。

---

## 1. Scientific problem：它声称解决的科学问题是什么？

黑盒奖励优化倾向模式坍缩，但科学发现常需要从多个高奖励模式采样并估计多解分布。
...
在有向构造图上学习前向/后向流，使终止对象的边缘概率与正奖励成比例。
...
组合对象构造轨迹、终点奖励；可用 off-policy 采样与不同 flow consistency 目标训练。
...
把局部构造转移的流守恒与终点奖励连接，令多个生成路径的流量聚合到对象概率。
...
理论与多类组合任务支持 reward-proportional sampling，并解释 trajectory balance 等训练目标。
...
| 科学结论 | 论文实际验证链覆盖到的最远层级 | 不能声称 reward-proportional 就等于 Boltzmann 物理分布，除非 reward、温度和状态测度有严格定义。 |
...
在连续高维晶体中，离散构造和终点归一困难；观察到的多样性也可能来自温度/熵正则，而非 flow matching。
...
熵正则 RL、top-k/quality-diversity、MCMC、archive sampling，以及相同 oracle 预算下的覆盖比较。
...
若在同一 reward 与预算下简单温度采样/质量多样性算法达到相同 basin 分布，GFlowNet 的额外复杂度没有必要。
...
它改变了优化目标：从 argmax 转为与 reward 成比例的多模态采样，是多解科学设计的重要抽象。
...
晶体中可把离散全局结构假设或 basin archive 作为对象空间，但连续坐标、弛豫后归并和不确定 reward 需新方法。
...
基础思想非常契合多晶型发现，但直接套用并不构成贡献；无机晶体的混合离散—连续、等价性和多保真问题才是难点。

### 对当前晶体主线的关系

- **应当怎样使用：** 作为 energy-only GRPO 的替代目标候选，用于保持低能 basin 的概率覆盖。
- **不应怎样引用：** 不能声称 reward-proportional 就等于 Boltzmann 物理分布，除非 reward、温度和状态测度有严格定义。
- **最值得立即做的验证：** 在已知多盆地的小体系上比较 GFlowNet、熵正则 RL、archive reweighting 对目标 basin 分布的 KL 与发现效率。
