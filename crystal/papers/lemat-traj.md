...
category: "数据/轨迹"
audit_role: "数据标准化"
paper_url: "https://arxiv.org/abs/2508.20875"
project_url: "https://github.com/LeMaterial/lematerial-fetcher"
...
| 主题 | 数据/轨迹 |
| 在领域中的角色 | 数据标准化 |
| 论文 | [论文地址](https://arxiv.org/abs/2508.20875) |
| 项目 | [项目 / 代码](https://github.com/LeMaterial/lematerial-fetcher) |
| 审计状态 | 开放数据/工具发布；跨泛函混用需谨慎。 |

## 0. 三十秒结论

**论文做了什么：** 统一 Materials Project、Alexandria、OQMD 等来源的 1.2 亿余轨迹构型，协调多种 DFT 泛函和元数据。

**去故事化判断：** 对我们极重要，但不能把下载轨迹数据本身当作方法创新。

**对当前研究最直接的用途：** 用于训练局部物理修正、定义 basin transition、分析 force/energy 分布。

---

## 1. Scientific problem：它声称解决的科学问题是什么？

量子轨迹数据碎片化、格式不一，使模型难以利用终态之外的高能、高力和弛豫过程。
...
输入结构，输出能量/力监督或轨迹序列；可用于 MLIP、局部修正器和生成路径建模。
...
聚合多个数据库和 PBE/PBEsol/SCAN/r2SCAN 等泛函；异质性既扩大覆盖，也引入不可直接比较的系统偏差。
...
标准化 schema、过滤质量、保留轨迹上下文，并提供可扩展抓取工具。
...
对力预测和弛豫任务的微调收益支持轨迹价值；尚不能自动证明轨迹可让生成器学会跨盆地搜索。
...
| 科学结论 | 论文实际验证链覆盖到的最远层级 | 不能跨泛函直接混合绝对能量，也不能把几何优化轨迹当作有限温度动力学。 |
...
收益可能只是更多结构覆盖，而不是“轨迹顺序”信息；随机打乱轨迹也许同样有效。
...
相同构型集合但去掉时间顺序、只保留终点+随机噪声、单泛函子集。
...
固定所有结构，比较有序轨迹、打乱轨迹和无序集合监督；若相同，则真正贡献是数据覆盖而非 dynamics。
...
大型轨迹数据整合贡献强；对生成方法的意义取决于是否利用顺序、力与盆地信息。
...
迁移抽象是区分“构型覆盖”与“轨迹因果顺序”，并为二者设计独立消融。
...
对我们极重要，但不能把下载轨迹数据本身当作方法创新。

### 对当前晶体主线的关系

- **应当怎样使用：** 用于训练局部物理修正、定义 basin transition、分析 force/energy 分布。
- **不应怎样引用：** 不能跨泛函直接混合绝对能量，也不能把几何优化轨迹当作有限温度动力学。
- **最值得立即做的验证：** 构建同组成多起点轨迹子集，标注 relaxed basin id，测有序轨迹对 basin-aware 生成的增益。
