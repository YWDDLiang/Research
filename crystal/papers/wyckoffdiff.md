...
category: "对称离散生成"
audit_role: "表示/离散机制"
paper_url: "https://arxiv.org/abs/2502.06485"
project_url: "https://github.com/httk/wyckoffdiff"
...
| 主题 | 对称离散生成 |
| 在领域中的角色 | 表示/离散机制 |
| 论文 | [论文地址](https://arxiv.org/abs/2502.06485) |
| 项目 | [项目 / 代码](https://github.com/httk/wyckoffdiff) |
| 审计状态 | 已发表；官方代码已核验。 |

## 0. 三十秒结论

**论文做了什么：** 在 protostructure/Wyckoff 表示上进行离散扩散，使对称性按构造成立，并提出 Fréchet Wrenformer Distance 衡量对称分布。

**去故事化判断：** 强对称路线论文，但与我们强调的未知多盆地探索存在潜在上限冲突。

**对当前研究最直接的用途：** 作为 Wyckoff 离散模型与 fast proposal 基线。

---

## 1. Scientific problem：它声称解决的科学问题是什么？

逐原子连续生成难以稳定产生空间群对称，且高维冗余；能否直接生成压缩的对称描述。
...
输入噪声离散状态，输出元素—Wyckoff 占位等 protostructure，再经下游坐标实现/弛豫得到晶体。
...
训练依赖可靠的 Wyckoff 分解；连续自由参数和后续几何质量可能不在离散模型内充分解决。
...
设计适配集合/占位的离散扩散网络，硬编码对称合法性，并用新的嵌入距离评估生成分布。
...
快速生成、对称有效性和分布指标支持表示优势；最终稳定性仍受结构实例化与 relaxer 影响。
...
| 科学结论 | 论文实际验证链覆盖到的最远层级 | 不能把 protostructure 新颖等同于完整结构新颖，不能忽略连续实例化失败。 |
...
结果可能来自压缩搜索空间与后处理，而非离散扩散；新指标可能偏好相同表示族。
...
Wyckoff 频率模型、无扩散的 autoregressive set model、相同 downstream relaxer 的随机 protostructure。
...
固定生成的 Wyckoff protostructure，替换离散模型；并在对称破缺目标上测上限。若简单频率模型等效，网络贡献有限。
...
迁移价值是先生成低维等价类/结构骨架，再把连续几何交给第二阶段。
...
强对称路线论文，但与我们强调的未知多盆地探索存在潜在上限冲突。

### 对当前晶体主线的关系

- **应当怎样使用：** 作为 Wyckoff 离散模型与 fast proposal 基线。
- **不应怎样引用：** 不能把 protostructure 新颖等同于完整结构新颖，不能忽略连续实例化失败。
- **最值得立即做的验证：** 用同一批 protostructure 比较不同 continuous decoder，并分析其可达低能盆地集合。
